...
st.caption("Interactive multi-asset portfolio, risk, controls & NFR analytics")
...
cc1.metric("Controls Passed", f"{control_summary['passed']}/{control_summary['total_controls']}")
...
nr2.metric(
    ...
    int(risk_register["Inherent Rating"].isin(["High", "Critical"]).sum()),
)
nr3.metric(
    "High/Critical Residual",
    int(risk_register["Residual Rating"].isin(["High", "Critical"]).sum()),
)
...
st.caption(
    "Educational NFR-style framework using RCSA-style scoring and project-defined KRIs. "
    "It is not a bank's proprietary risk methodology or a regulatory compliance system."
)
...
ending_diff = current["portfolio_wealth"].iloc[-1] - current["benchmark_wealth"].iloc[-1]
st.caption(f"Historical ending-value difference vs. 60/40: ${ending_diff:,.0f}")
...
stress_display["period_return"] = stress_display["period_return"].map(lambda x: f"{x:.2%}")
stress_display["max_drawdown"] = stress_display["max_drawdown"].map(lambda x: f"{x:.2%}")
stress_display["recovery_date"] = stress_display["recovery_date"].apply(
    lambda x: "Not recovered by scenario end"
    if pd.isna(x)
...
sens_display["Ending Value"] = sens_display["Ending Value"].map(lambda x: f"${x:,.0f}")
...
- The NFR section is an educational framework and not a bank's proprietary methodology.
"""
    )
